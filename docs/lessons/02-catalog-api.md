...
SKU, barcode, and category-name uniqueness remain case-sensitive, matching the first migration. Category renaming updates product reads through a join. Products are deactivated instead of deleted, preserving their identity for future movement history. This describes the catalog milestone. Authentication and store authorization now wrap these routes; see lesson 3. Public deployment remains a later milestone.
