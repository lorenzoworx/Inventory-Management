...
const kindLabel = { OPENING: "Opening balance", SALE: "Sale", ADJUSTMENT: "Adjustment", PURCHASE: "Purchase receipt", TRANSFER_OUT: "Transfer dispatch", TRANSFER_IN: "Transfer receipt", REORDER: "Reorder point" };
type EntryKind = Exclude<keyof typeof kindLabel, "PURCHASE" | "TRANSFER_OUT" | "TRANSFER_IN">;
  ...
  return <>
    ...
    {locations.state.phase === "ready" && <>
      <form className="filters stock-filters catalog-panel" key={params.toString()} onSubmit={filter}>
        ...
        {history && <label><span id="movement-kind-label">Movement type</span><select aria-labelledby="movement-kind-label" name="kind" defaultValue={params.get("kind") ?? ""}><option value="">All movements</option>{(["OPENING", "SALE", "ADJUSTMENT", "PURCHASE", "TRANSFER_OUT", "TRANSFER_IN"] as const).map((kind) => <option key={kind} value={kind}>{kindLabel[kind]}</option>)}</select></label>}
