    ...
    <div className="category-layout">
      <section className="catalog-panel" aria-label="Categories">
        ...
        {categories.state.phase === "ready" && <>
          {categories.state.data.items.length === 0 ? <p className="empty-state">No categories on this page.</p> : <ul className="category-list">{categories.state.data.items.map((category) => <li key={category.id}><span>{category.name}</span><button className="text-button" disabled={busy} aria-label={`Edit ${category.name}`} onClick={() => { setError(""); setEditing(category); }}>Edit</button></li>)}</ul>}
          <Pagination {...categories.state.data} changePage={(page) => setParams({ page: String(page) })} />
        </>}
      </section>
      <form className="catalog-panel category-form" key={editing?.id ?? "new"} onSubmit={(event) => { void submit(event); }} noValidate>
